...
const Courses = ({ state, dispatch }) => {
  ...
  return (
    <>
      ...
      <p>
        <img loading="lazy" className="left-aligned-image" src={`${baseUrl}/images/Courses/courses01.jpg`} alt="placeholder" />
      ...
      <div className="container-left-top">
        {state.courses.map((course, i) => (
          <div key={i} className="container-link" style={{ backgroundImage: course.isSelected ? "linear-gradient(to right bottom, lightpink, lightyellow)" : themeData.backgroundImage, border: themeData.border }} onClick={() => dispatch({ type: "SELECT_COURSE", index: i })}>
      ...
      >
        <img loading="lazy" className="slider-close" src={`${baseUrl}/images/close.png`} alt="close" onClick={() => dispatch({ type: "DESELECT_COURSE" })} />
