...
const Greet = ({ state, dispatch, scrollToEvents, scrollToNews }) => {
  ...
  const userInputChange = (e) => {
    ...
    setUser({
      ...updatedUser,
      error: name === "mobile" && mobiles.includes(value) ? "User with mobile already exists !" : name === "email" && emails.includes(value) ? "User with email already exists !" : "",
    });
  };
      ...
      <div className="greet-user">
        <div className="user-info">
          {unreadEnquiries.length > 0 && (
            <div className="notification" onClick={() => handleViewEnquiries()}>
